...
本儲存庫以 [`/.github/labels.yml`](.github/labels.yml) 為單一來源，定義 **45 個全正體中文標準標籤**（命名格式：中文前綴 + 半形冒號空格），並由工作流程自動同步至組織內所有儲存庫：

| 維度 | 標籤範例 | 用途 |
|------|---------|------|
| **類型** | `類型: 錯誤`、`類型: 功能`、`類型: 效能`、`類型: 安全` | 工作的性質 |
| **領域** | `領域: 前端`、`領域: 後端 API`、`領域: 資料庫`、`領域: 金流與結帳` | 所屬系統元件 |
| **優先** | `優先: P0`、`優先: P1`、`優先: P2`、`優先: P3` | 緊急程度（未貼即未排程） |
| **狀態** | `狀態: 待分類`、`狀態: 進行中`、`狀態: 等待審查`、`狀態: 受阻` | 工作流程狀態 |
| **環境** | `環境: 開發`、`環境: 測試`、`環境: 正式` | 發生環境 |
| **無效 / 標記** | `無效: 重複`、`標記: 破壞性變更`、`標記: 線上事故` | 結案原因與跨維度旗標 |
